...
//tectonic markers parsing
d3.json(tecPlates, function (response) {
    ...
    for (var i=0; i<response.features.length; i++) {
        var location = response.features[i];
        //console.log(location);
        console.log(location.properties.LAYER);
        console.log(location.geometry.coordinates);
        console.log(location.properties.Name);
        console.log(location.properties.Source);
        console.log(location.properties.Type);
        console.log("location.geometry.coordinates");
        console.log(location.geometry.coordinates)
        var lat = 0;
        var lng = 0;
        if (location.properties.Type === "subduction") {
            latlngList= [];
            for ( var j = 0; j<location.geometry.coordinates.length; j++) {
                //console.log(location.geometry.coordinates[j]);
                for (var k = 0; k<location.geometry.coordinates[j].length; k=k+2) {
                    latlng =[];
                    console.log(location.geometry.coordinates[j][k]);
                    console.log(location.geometry.coordinates[j][k]);
            lat = location.geometry.coordinates[j][k];
            lng = location.geometry.coordinates[j][k+1];
            latlng.push(lng);
            latlng.push(lat);
            ...
            console.log(latlngList);
            tectonicLines.push(L.polyline([latlngList], {
                color: "yellow",
                thick: "10"
                //fillOpacity: opac,
                //fillColor: cl,
...
var erthqks = L.layerGroup(erthQuakeMarkers);
var tectonics = L.layerGroup(tectonicLines);
...
var overlayMaps = {
  "Earth Quake Map": erthqks,
  "Tectonic Map": tectonics
};
...
tectonics.addTo(myMap);
*/
//myMap.add(satelite, erthqks, tectonics);

/*
var myMap = L.map("map", {
    center: [32.7488333,-115.814],
    zoom: mapZoomLevel,
    layers: [satelite, erthqks, tectonics]
   });
